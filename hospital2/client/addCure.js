...
let oldVal
  ...
  let originalQuantity = oldVal // Variable to store the original quantity of the cure being edited
...
<button class="editButton" data-cure-id="${cure.id}">Edit</button></td>
    ...
    deleteButtons.forEach(button => {
      button.addEventListener('click', async () => {
        ...
        if (confirm('Are you sure you want to delete this cure?')) {  //thiere is a problem here need to solve becuase if the user hit canle it will mkae an error 
          try {
...
// }
    ...
    try {
      if (!isAdmin && editingCureId) {
        ...
        if (parseInt(cureQuantityInput.value) < originalQuantity) {
